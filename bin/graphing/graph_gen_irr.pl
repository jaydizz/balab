...
#} else { 
  my $curl = `curl  -G 'http://localhost:8086/query' --data-urlencode "db=test_measure" --data-urlencode "q=SELECT sum(\"value\") FROM announce WHERE time >= now()-30d AND time < now() GROUP BY time(1d), \"validity\" fill(none)"`;
  chomp $curl;
